...
const Step = ({ number, title, desc, icon, isLast }: StepProps) => (
  <div className="relative flex gap-8 md:gap-12 pb-12 last:pb-0 group">
    {!isLast && (
      <div className="absolute left-[27px] top-[60px] bottom-[-20px] w-px bg-white/10 group-hover:bg-zinc-500 transition-colors duration-500 hidden md:block" />
    )}
    <div className="relative z-10 flex-shrink-0 w-14 h-14 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md shadow-sm flex items-center justify-center text-zinc-400 group-hover:scale-110 group-hover:border-zinc-500 transition-all duration-500">
      {icon}
    ...
    <div className="flex-1 pt-2">
      <h3 className="text-xl font-black text-white mb-2 group-hover:text-zinc-400 transition-colors">{title}</h3>
...
const MarketplaceSection = () => {
  ...
  const steps = useMemo(() => {
    const raw = t("A4Network.steps", { returnObjects: true }) as
        ...
        >
          ...
          <div className={steps.length > 0 ? "w-full lg:w-1/3 shrink-0" : "w-full lg:w-1/2"}>
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white mb-8 leading-tight tracking-tight">
              {t("A4Network.titleLine1")}
              <span className="text-zinc-400">{t("A4Network.titleHighlight")}</span>
            </h2>
            <p className="text-lg text-slate-400 mb-10 font-medium leading-relaxed whitespace-pre-line">
              {t("A4Network.body")}
            </p>
            <GetInstantQuoteButton
              text={t("A4Network.cta")}
          ...
          >
            ...
            {steps.length > 0 ? (
              <div className="bg-[#0D0F18] backdrop-blur-xl p-8 md:p-16 rounded-[3rem] border border-white/15 shadow-2xl relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-tr from-zinc-600/[0.03] to-transparent pointer-events-none" />
